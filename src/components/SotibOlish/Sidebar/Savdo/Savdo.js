...
function Savdo({changeLink, link, savdooynasi, sidebaractive2, users}) {
    ...
    return (
        <div className={'row list'} id={'savdo'}>
            ...
            <ul>
                {
                    users.addTrade || users.getTradeAdmin || users.getTrade ?
                        <li onClick={sidebaractive}><NavLink to={'/main/tradeList'}
                                                             className={isActive => isActive ? 'active-enter-link li-text' : 'li-text'}>{t('Sidebar.19')}</NavLink>
                        </li> : ''
                }


                {
                    users.addTrade || users.getTrade ? <li onClick={savdod}><NavLink to={'/shopping'}
                                                                                     className={isActive => isActive ? 'active-enter-link li-text' : 'li-text'}>{t('Sidebar.21')}</NavLink>
                    </li> : ''
                }
                {
                    users.addLoss && <li onClick={sidebaractive}><NavLink to={'/main/addLossProducts'}
                                                                          className={isActive => isActive ? 'active-enter-link li-text' : 'li-text'}>{t('Sidebar1.mahsulotyuqotish')}</NavLink>
                    </li>
                }

                {
                    users.getTrade || users.getTradeAdmin ?
                        <li onClick={sidebaractive}><NavLink to={'/main/lossProducts'}
                                                             className={isActive => isActive ? 'active-enter-link li-text' : 'li-text'}>{t('Sidebar1.yoqotilganmahsulot')}</NavLink>
                        </li>:''
                }
...
export default connect((users), {active, savdooynasi})(Savdo)
